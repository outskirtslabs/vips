...
(defn- supported-object-arg?
  [{:keys [value-type]}]
  (= "VipsImage" value-type))

(defn- supported-boxed-input?
  [{:keys [value-type]}]
...
(defn- supported-input?
  [{:keys [kind] :as arg}]
  (case kind
    :object (supported-object-arg? arg)
    :boxed (supported-boxed-input? arg)
...
(defn- operation-spec
  ...
  (let [required-inputs* (required-inputs operation)
    ...
    {:id              (operation-id (:name operation))
     ...
     :required-inputs (mapv #(select-keys % [:name :blurb :kind :value-type]) required-inputs*)
     :optional-inputs (mapv #(select-keys % [:name :blurb :kind :value-type]) optional-inputs*)
     :outputs         (mapv #(select-keys % [:name :blurb :kind :value-type]) outputs*)}))
...
(defn generate!
  []
  (v/init!)
  (let [{:keys [operations skipped]} (load-operation-infos)
        operations                   (->> operations
                                          (filter operation-supported?)
                                          (map operation-spec)
                                          (sort-by :id)
                                          vec)
        enum-type-names              (->> operations
                                          (mapcat (fn [{:keys [required-inputs optional-inputs outputs]}]
                                                    (concat required-inputs optional-inputs outputs)))
                                          (filter #(= :enum (:kind %)))
                                          (map :value-type)
                                          distinct
                                          sort
