...
* https://github.com/outskirtslabs/vips/blob/main/examples/image_diff.clj[`examples/image_diff.clj`] compares two same-sized images, thresholds the absolute difference, and composites a red highlight overlay onto the changed pixels.
* https://github.com/outskirtslabs/vips/blob/main/examples/palette_extractor.clj[`examples/palette_extractor.clj`] extracts the top dominant colors from an image with `hist_find_ndim`, prints channel stats from `stats`, and renders a swatch strip below the source image.
...
https://www.libvips.org/API/8.18/func.block_untrusted_set.html[`vips_block_untrusted_set`
